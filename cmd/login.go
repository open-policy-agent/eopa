...
func writeConfig(config *viper.Viper, logger logging.Logger) error {
	path := ".styra.yaml"
	cfg, err := generatedConfig(config)
	if err != nil {
		return err
	}
...
const configHdr = "# generated by `eopa login`\n"

func generatedConfig(config *viper.Viper) ([]byte, error) {
	output := bytes.Buffer{}
	output.WriteString(configHdr)
	bs, err := yaml.Marshal(map[string]any{
		dasURL: trimURL(config.GetString(dasURL)),
		"data": []any{
			config.GetString(targetDir),
		},
	})
	if err != nil {
