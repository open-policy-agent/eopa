...
import (
	...
	"github.com/styrainc/load/pkg/store/internal/errors"
...
func TestInMemoryWrite(t *testing.T) {
	...
	for i, tc := range tests {
		...
		err := storage.WriteOne(ctx, store, op, storage.MustParsePath(tc.path), bjson.MustNew(value))
		if tc.expected == nil {
			if err != nil {
		...
		result, err := readOne(ctx, store, storage.MustParsePath(tc.getPath))
...
func TestInMemoryWriteOfStruct(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			actual, err := readOne(ctx, store, storage.MustParsePath("/x"))
			if err != nil {
				t.Fatal(err)
			}
...
func TestInMemoryTxnMultipleWrites(t *testing.T) {
	...
	for _, w := range writes {
		...
		if err := store.Write(ctx, txn, w.op, storage.MustParsePath(w.path), bjson.MustNew(jsn)); err != nil {
...
func TestInMemoryTxnWriteFailures(t *testing.T) {
	...
	for _, w := range writes {
		...
		err := store.Write(ctx, txn, w.op, storage.MustParsePath(w.path), bjson.MustNew(jsn))
...
func TestInMemoryTxnReadFailures(t *testing.T) {
	...
	if err := store.Write(ctx, txn, storage.RemoveOp, storage.MustParsePath("/c/0/y"), bjson.MustNew(nil)); err != nil {
...
func TestInMemoryTxnBadWrite(t *testing.T) {
	...
	store := NewFromObject(loadSmallTestData())
	txn := storage.NewTransactionOrDie(ctx, store)
	if err := store.Write(ctx, txn, storage.RemoveOp, storage.MustParsePath("/a"), bjson.MustNew(nil)); !storage.IsInvalidTransaction(err) {
...
func TestInMemoryTriggers(t *testing.T) {
	...
	if err := store.Write(ctx, writeTxn, storage.ReplaceOp, modifiedPath, bjson.MustNew(expectedValue)); err != nil {
		t.Fatalf("Unexpected write error: %v", err)
	}
	...
	expData := storage.DataEvent{Path: modifiedPath, Data: bjson.MustNew(expectedValue), Removed: false}
...
func TestInMemoryTriggersUnregister(t *testing.T) {
	...
	writeTxn = storage.NewTransactionOrDie(ctx, store, storage.WriteParams)
	if err := store.Write(ctx, writeTxn, storage.AddOp, modifiedPath, bjson.MustNew(expectedValue)); err != nil {
...
func loadSmallTestData() bjson.Object {
