...
func TestDynamoDBGet(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result:              "",
			error:               "eval_builtin_error: dynamodb.get: api error ValidationException: One or more parameter values were invalid: Type mismatch for key",
			doNotResetCache:     false,
			time:                now,
			interQueryCacheHits: 0,
		},
...
func TestDynamoDBQuery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result:              "",
			error:               "eval_builtin_error: dynamodb.query: api error ValidationException: One or more parameter values were invalid: Condition parameter type does not match schema type",
			doNotResetCache:     false,
			time:                now,
			interQueryCacheHits: 0,
		},
