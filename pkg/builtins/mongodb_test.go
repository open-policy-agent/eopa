...
func TestMongoDBSend(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			`p := mongodb.send({})`,
			"",
			`eval_type_error: mongodb.send: operand 1 missing required request parameters(s): {"find", "uri"}`,
			false,
			now,
			0,
		},
		{
			"non-existing doc query",
		...
		},
		{
			...
			`{{"result": {"p": [{"documents": [{"foo": "x"}]}, {"documents": [{"foo": "x"}]}]}}}`,
			"",
			false,
			now,
			0,
...
func startMongoDB(t *testing.T) (testcontainers.Container, string) {
	...
	if _, err := collection.InsertOne(ctx, bson.D{{Key: "foo", Value: "x"}}); err != nil {
		t.Fatal(err)
	}
