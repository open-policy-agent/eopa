...
func TestPatch(t *testing.T) {
	// Taken from https://github.com/json-patch/json-patch-tests (Apache 2.0)
	// The tests in the end for create are ours.
	const jsonSuite = `
	[
	    ...
	    { "comment": "create behaves like add for empty path",
	      "doc": {},
	      "patch": [{"op": "create", "path": "", "value":1}],
	      "expected": 1 },

	    { "comment": "create creates any missing enclosing object",
	      "doc": {},
	      "patch": [{"op": "create", "path": "/a/b", "value":1}],
	      "expected": {"a": {"b": 1}} },

	    { "comment": "create behaves like add if only the last path segment doesn't exist",
	      "doc": {"a": {}},
	      "patch": [{"op": "create", "path": "/a/b", "value":1}],
	      "expected": {"a": {"b": 1}} },

	    { "comment": "create behaves like add if path exists",
	      "doc": {"a": {"b": 0}},
	      "patch": [{"op": "create", "path": "/a/b", "value":1}],
	      "expected": {"a": {"b": 1}} },

	    { "comment": "create behaves like add with array",
	      "doc": [0],
	      "patch": [{"op": "create", "path": "/1", "value":1}],
	      "expected": [0,1] },

	    { "comment": "create behaves like add with array",
	      "doc": [0],
	      "patch": [{"op": "create", "path": "/0", "value":1}],
	      "expected": [1,0] }
	]`
	...
	if err := json.Unmarshal([]byte(jsonSuite), &tests); err != nil {
		t.Fatal(err)
	}
	...
	for i, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			jdoc, err := New(test.Doc)
			jdocCopy := jdoc.Clone(true).(Json)
			if err != nil {
				t.Fatal(err)
			}
			jexpected, err := New(test.Expected)
			if err != nil {
				t.Fatal(err)
			}
			jpatched, err := patch.ApplyTo(jdoc)
			if jdocCopy.Compare(jdoc) != 0 {
			...
			} else if !test.Disabled && jexpected.Compare(jpatched) != 0 {
				t.Errorf("%v != %v", jexpected, jpatched)
			}

			// Check the encoding/json serialization and deserialization works.
			...
			var patch2 Patch
			if err := patch2.UnmarshalJSON(serialized); err != nil {
				if test.Error != "" {
					return
				}
				t.Fatal(err)
...
func TestPatchBinary(t *testing.T) {
	tests := []struct {
		...
		{
			"Add adding a new binary object property",
			NewObject(map[string]File{
				"a": NewString("foo"),
			}),
			Op{
				Op:          PatchOpAdd,
				Path:        "/b",
				valueBinary: NewBlob([]byte("bar")),
			},
			NewObject(map[string]File{
				"a": NewString("foo"),
				"b": NewBlob([]byte("bar")),
