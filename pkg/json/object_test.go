...
func TestObjectSizes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run("", func(t *testing.T) {
			...
			for i := 0; i < obj.Len(); i++ {
				if !reflect.DeepEqual(NewFloatInt(int64(i+1)), obj.Iterate(i)) {
					t.Error("broken iteration")
				}

				if !reflect.DeepEqual(NewFloatInt(int64(i+1)), obj.Value(fmt.Sprintf("%02d", i))) {
					t.Error("broken value")
				}

				e, err := obj.Extract(fmt.Sprintf("/%02d", i))
				if err != nil || !reflect.DeepEqual(NewFloatInt(int64(i+1)), e) {
